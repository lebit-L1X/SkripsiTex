...
Pendekatan yang digunakan dalam penelitian ini mengadaptasi model \textit{EEG Data Augmentation using Variational Autoencoder} [32], yaitu suatu model generatif berbasis VAE yang dirancang untuk menghasilkan data EEG sintetis yang menyerupai karakteristik data asli. Model ini pada dasarnya terdiri dari komponen \textit{encoder} dan \textit{decoder} yang dilatih secara end-to-end untuk merekonstruksi sinyal melalui representasi laten probabilistik.

Dalam implementasinya, arsitektur awal model tersebut mengalami penyesuaian untuk menyesuaikan dengan kebutuhan penelitian. Secara khusus, dimensi data dan struktur jaringan dimodifikasi dari pendekatan awal menjadi berbasis \textit{1D Convolutional Neural Network} (1D CNN). Ketetapan didasarkan pada penelitian referensi [31] yang memproses sinyal EEG sebagai kanal terpisah dalam domain satu dimensi, sehingga lebih sesuai dengan karakteristik data serta kompatibel dengan \textit{pipeline} klasifikasi yang digunakan. Dengan demikian, arsitektur yang diusulkan tetap mempertahankan prinsip dasar VAE, namun diadaptasi agar selaras dengan representasi data yang digunakan.
...
Hasil tahap tersebut digunakan dalam dua skenario klasifikasi yang berbeda. Pada skenario pertama, proses klasifikasi dilakukan hanya menggunakan data asli, sedangkan pada skenario kedua, proses klasifikasi memanfaatkan kombinasi data asli dan data hasil augmentasi. Kedua skenario tersebut menggunakan metode seleksi fitur \textit{minimum Redundancy Maximum Relevance} (mRMR) serta algoritma klasifikasi XGBoost berdasarkan referensi penelitian sebelumnya [31]. Perbandingan antara kedua skenario ini bertujuan untuk mengevaluasi pengaruh penggunaan data sintetis terhadap performa model klasifikasi yang dihasilkan.
