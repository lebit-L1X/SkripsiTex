...
Data sintetis yang dihasilkan kemudian digabungkan dengan data EEG asli sebagai bentuk \textit{data augmentation}. Dataset gabungan selanjutnya melalui proses dekomposisi pita frekuensi EEG meliputi delta, theta, alpha, beta, dan gamma menggunakan \textit{bandpass filter}.

Setelah proses dekomposisi frekuensi, setiap kanal distandardisasi menggunakan \textit{StandardScaler} untuk menjaga konsistensi distribusi data antar kanal dan pita frekuensi. Tahap berikutnya dilakukan ekstraksi fitur statistik untuk menghasilkan representasi fitur berdimensi tinggi yang kemudian digunakan dalam proses seleksi fitur menggunakan \textit{mRMR} serta klasifikasi menggunakan \textit{XGBoost} dan \textit{Deep Neural Network}.
...
Model \textit{Deep Neural Network} (DNN) yang digunakan dalam penelitian ini merupakan model \textit{feedforward neural network} sederhana yang terdiri dari tiga lapisan utama, yaitu lapisan masukan, satu lapisan tersembunyi, dan lapisan keluaran. Lapisan tersembunyi terdiri dari 5 neuron dengan fungsi aktivasi \textit{log-sigmoid} yang digunakan untuk memperkenalkan non-linearitas serta membatasi keluaran neuron pada rentang (0,1). Lapisan keluaran menggunakan fungsi aktivasi \textit{softmax} yang menghasilkan distribusi probabilitas untuk masing-masing kelas sesuai dengan jumlah kelas (\textit{n\_classes}).
...
Gambar \ref{fig:dnn_architecture} menunjukkan arsitektur model DNN yang digunakan dalam penelitian ini, yang terdiri dari lapisan masukan, satu \textit{hidden layer} dengan 5 neuron, serta lapisan keluaran dengan fungsi aktivasi \textit{softmax}. Struktur ini digunakan sebagai model klasifikasi berbasis \textit{supervised learning} untuk memetakan fitur EEG yang telah diekstraksi ke dalam kelas target.
...
Evaluasi kinerja model dilakukan menggunakan data uji dengan metrik akurasi dan \textit{Area Under Curve} (AUC). Probabilitas keluaran model kemudian dikonversi menjadi label kelas menggunakan operasi argmax. Selanjutnya, performa model dianalisis lebih lanjut menggunakan confusion matrix serta \textit{classification report} yang mencakup nilai precision, recall, dan F1-score.
