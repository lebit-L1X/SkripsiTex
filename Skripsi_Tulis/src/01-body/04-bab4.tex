...
\begin{enumerate}
\item \textbf{Dataset:}
    Eksperimen augmentasi menggunakan dataset EEG ASD dari penelitian \cite{eeg_dataset_asd}. Data EEG asli digunakan sebagai data masukan untuk melatih model \textit{Variational Autoencoder} (VAE) dalam menghasilkan sinyal EEG sintetis. Sebelum digunakan pada proses pelatihan model VAE, sinyal EEG melalui tahapan \textit{preprocessing} yang mengacu pada penelitian referensi \cite{paper_ref}, meliputi \textit{filtering}, segmentasi sinyal, serta proses pengurangan artefak untuk meningkatkan kualitas sinyal EEG. Pada eksperimen augmentasi, data EEG dipisahkan berdasarkan kanal dan kelas data (NORMAL dan ASD) sebelum proses pelatihan model. Dengan demikian, proses augmentasi dilakukan pada 32 subset data yang berbeda yang merepresentasikan kombinasi setiap kanal dan kelas.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{assets/pics/time_domain.png}
    \caption{Visualisasi Domain Waktu Sampel Sinyal EEG Sintetis}
    \label{fig:time_domain}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{assets/pics/freq_domain.png}
    \caption{Visualisasi Domain Frekuensi Sampel Sinyal EEG Sintetis}
    \label{fig:freq_domain}
\end{figure}

Karakteristik tersebut juga terlihat pada visualisasi domain waktu dan domain frekuensi pada \ref{fig:time_domain} dan \ref{fig:freq_domain}, di mana sinyal sintetis mampu mengikuti pola amplitudo utama serta distribusi spektral frekuensi dominan dari sinyal asli.
...
Pada eksperimen ini digunakan dua model klasifikasi, yaitu \textit{Deep Neural Network} (DNN) sebagai model pembanding berdasarkan penelitian rujukan dan \textit{Extreme Gradient Boosting} (XGBoost) sebagai model yang diusulkan. Seluruh proses pelatihan dan pengujian dilakukan menggunakan data EEG asli yang telah melalui tahap \textit{preprocessing} sebagaimana dijelaskan pada bab sebelumnya. Evaluasi performa dilakukan menggunakan metrik \textit{accuracy}, \textit{sensitivity}, \textit{specificity}, \textit{precision}, \textit{F1-score}, dan \textit{Area Under Curve} (AUC).
...
Selain membandingkan performa model klasifikasi, penelitian ini juga memperluas studi sebelumnya melalui penerapan augmentasi data berbasis VAE. Penelitian \cite{gan_eeg_emotion} menunjukkan bahwa data sintetis yang dihasilkan oleh VAE mampu meningkatkan performa klasifikasi EEG pada tugas \textit{motor imagery}. Dalam penelitian tersebut, sinyal EEG direpresentasikan pada domain waktu dan diklasifikasikan menggunakan EEGNet, menghasilkan peningkatan akurasi dari 0.73 menjadi 0.88 setelah penambahan data sintetis. Hasil tersebut menunjukkan bahwa VAE mampu menghasilkan data yang cukup representatif untuk memperkaya variasi data pelatihan dan meningkatkan kemampuan model dalam mempelajari pola EEG.
