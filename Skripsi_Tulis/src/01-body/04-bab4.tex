...
Bagian ini menjelaskan konfigurasi eksperimen yang digunakan pada proses augmentasi data dan klasifikasi sinyal EEG. Konfigurasi eksperimen meliputi pengaturan dataset, parameter \textit{training} model, serta skenario pengujian yang digunakan selama penelitian untuk menjaga konsistensi dan validitas hasil eksperimen.
...
Eksperimen ini digunakan untuk menilai sejauh mana model VAE mampu mempertahankan karakteristik utama sinyal EEG serta menentukan kelayakan data sintetis yang dihasilkan untuk digunakan pada tahap augmentasi data dalam proses klasifikasi ASD dengan hasil evaluasi kualitas data sintetis untuk masing-masing kanal EEG pada kelas Normal dan ASD disajikan sebagai berikut.
...
Karakteristik tersebut juga terlihat pada visualisasi domain waktu dan domain frekuensi, di mana sinyal sintetis mampu mengikuti pola amplitudo utama serta distribusi spektral frekuensi dominan dari sinyal asli.
...
\begin{table}[H]
\centering
\caption{Perbandingan performa model ANN dan XGBoost Dengan Data EEG Asli}
\label{tab:ann_xgb_comparison}
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{lcc}
\toprule
\textbf{Metric} & \textbf{ANN} & \textbf{XGBoost} \\
\midrule
...
Recall & $0.6347 \pm 0.3089$ & $0.9724 \pm 0.0101$ \\
...
Hasil penelitian ini menunjukkan kesesuaian dengan penelitian terdahulu baik dari sisi performa model klasifikasi maupun efektivitas augmentasi data berbasis \textit{Variational Autoencoder} (VAE). Pada penelitian rujukan \cite{paper_ref}, model XGBoost terbukti memberikan performa yang jauh lebih baik dibandingkan model ANN pada klasifikasi ASD berbasis EEG, dengan nilai AUC sebesar $0.9963 \pm 0.0027$ dan \textit{F1-score} sebesar $0.9743 \pm 0.0093$, sedangkan model ANN hanya memperoleh AUC sebesar $0.6160 \pm 0.1861$ dan \textit{F1-score} sebesar $0.5638 \pm 0.2469$. Temuan yang hampir identik juga diperoleh pada penelitian ini. Pada pengujian menggunakan data EEG asli, model XGBoost menghasilkan AUC sebesar $0.9965 \pm 0.0025$ dan \textit{F1-score} sebesar $0.9751 \pm 0.0089$, sedangkan model DNN memperoleh AUC sebesar $0.6204 \pm 0.1827$ dan \textit{F1-score} sebesar $0.5689 \pm 0.2423$. Kemiripan hasil tersebut menunjukkan bahwa metodologi klasifikasi yang digunakan pada penelitian ini berhasil mereproduksi karakteristik performa yang dilaporkan pada penelitian rujukan, sekaligus memperkuat temuan bahwa XGBoost lebih efektif dalam menangani karakteristik data EEG dibandingkan model \textit{neural network} yang digunakan sebagai \textit{baseline}.

Selain membandingkan performa model klasifikasi, penelitian ini juga memperluas studi sebelumnya melalui penerapan augmentasi data berbasis VAE. Penelitian \cite{gan_eeg_emotion} menunjukkan bahwa data sintetis yang dihasilkan oleh VAE mampu meningkatkan performa klasifikasi EEG pada tugas \textit{motor imagery}. Dalam penelitian tersebut, sinyal EEG direpresentasikan pada domain waktu dan diklasifikasikan menggunakan EEGNet, menghasilkan peningkatan akurasi dari 0.73 menjadi 0.88 setelah penambahan data sintetis. Hasil tersebut menunjukkan bahwa VAE mampu menghasilkan data yang cukup representatif untuk memperkaya variasi data \textit{training} dan meningkatkan kemampuan model dalam mempelajari pola EEG.

Temuan pada penelitian ini menunjukkan kecenderungan yang serupa meskipun diterapkan pada permasalahan, representasi data, dan model klasifikasi yang berbeda. Berbeda dengan penelitian sebelumnya yang berfokus pada klasifikasi \textit{motor imagery} menggunakan representasi domain waktu, penelitian ini menerapkan VAE pada klasifikasi ASD berbasis EEG serta melakukan evaluasi pada domain waktu dan frekuensi. Setelah penambahan data sintetis, model XGBoost mengalami peningkatan performa yang konsisten, dengan nilai AUC meningkat dari $0.9965$ menjadi $0.9996$ dan \textit{F1-score} meningkat dari $0.9751$ menjadi $0.9924$. Hasil tersebut menunjukkan bahwa data sintetis yang dihasilkan masih mampu mempertahankan informasi diskriminatif yang relevan sehingga dapat dimanfaatkan secara efektif oleh model klasifikasi.

Namun demikian, penelitian ini juga menunjukkan bahwa manfaat augmentasi tidak selalu muncul secara konsisten pada seluruh model klasifikasi. Berbeda dengan penelitian \cite{gan_eeg_emotion} yang hanya mengevaluasi satu model klasifikasi, penelitian ini menunjukkan bahwa respons terhadap augmentasi sangat dipengaruhi oleh karakteristik model yang digunakan. Pada model DNN, peningkatan performa tidak terjadi secara merata setelah augmentasi dilakukan. Meskipun nilai presisi meningkat, sensitivitas dan \textit{F1-score} justru mengalami penurunan yang cukup signifikan. Temuan tersebut mengindikasikan bahwa efektivitas augmentasi tidak hanya dipengaruhi oleh kualitas data sintetis yang dihasilkan, tetapi juga oleh kemampuan model dalam memanfaatkan variasi tambahan yang diperoleh dari data hasil augmentasi.

Secara keseluruhan, hasil penelitian ini mendukung temuan penelitian terdahulu mengenai keunggulan XGBoost untuk klasifikasi ASD berbasis EEG serta potensi VAE sebagai metode augmentasi data EEG. Di sisi lain, penelitian ini memberikan temuan tambahan bahwa keberhasilan augmentasi berbasis VAE sangat bergantung pada karakteristik model klasifikasi yang digunakan, sehingga peningkatan performa tidak dapat diasumsikan terjadi secara merata pada seluruh algoritma klasifikasi.
...
Hasil penelitian ini menunjukkan beberapa peluang pengembangan yang menarik untuk diteliti lebih lanjut. Salah satu temuan utama adalah adanya perbedaan kualitas data sintetis antar kelas dan kanal EEG. Temuan tersebut mengindikasikan bahwa karakteristik distribusi data asli kemungkinan memiliki pengaruh terhadap kemampuan model VAE dalam mempelajari representasi laten dan menghasilkan data sintetis yang berkualitas. Oleh karena itu, hubungan antara kompleksitas distribusi data EEG dan kualitas hasil generasi menjadi aspek yang menarik untuk dieksplorasi pada penelitian berikutnya.
